...
@Component({
  selector: 'app-stats-ws',
  templateUrl: './stats-ws.component.html',
  styleUrls: ['./stats-ws.component.scss']
})
export class StatsWsComponent implements OnInit {
  data = [];
  dataLabels = [];
  isitNull = true;
  ...
  ngOnInit() {
    ...
    this.admin.statsWS().subscribe((res) => {
      // console.log(res)
      for (let i in res) {
        this.data.push(res[i]);
      }

      this.admin.morestatsWS().subscribe((result) => {
        console.log(result);
        let dataLabels;
        dataLabels = (result);
        for (let i = 0; i < 12; i++) {
